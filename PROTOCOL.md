...
Version: **1.0.0-alpha** *(archive — see [`ARCHIVED.md`](ARCHIVED.md))*

> Originally declared as `Version: 1`. Renamed at archive time to a SemVer pre-release tag (`v1.0.0-alpha`) to honestly signal that this version was withdrawn before formal ratification. Body text below speaks about "protocol version 1" / `protocol=1` in `INFO` advertisement as it was originally written; that is preserved as historical language. **Do not implement against this version** — it has been superseded by Protocol 2.0+.
